...
export function useCF (options: FamilyPayload): FamilyReturn {
  ...
  const eligibleChilds = computed((): Childrens[] | undefined =>
  ...
  const customTranchMessage = computed((): string => {
    const arr = [
  ...
  const customTranchRange = computed((): number[] =>
    options.salaries && options.salaries.length < 2 ? SALARY_SUPP_TRANCHS : SALARIES_SUPP_TRANCHS
  )
  ...
  const familySupplement = computed((): number => {
    if (eligibleChilds.value && eligibleChilds.value.length < 3) {
      return 0
    }
  ...
  const resume = computed((): string[] => {
    ...
    if (eligibleChilds.value && eligibleChilds.value.length < 3) {
      messages.push('Vous devez avoir au moins 3 enfants agé entre 3 et 21 ans')
    }
    if (eligibleChilds.value && eligibleChilds.value.length > 2) {
      messages.push(`Vous avez ${eligibleChilds.value.length} enfants scolarisé, vous êtes éligibles`)
    }
    if (familySupplement.value) {
      messages.push(`${total.value}€ forfaitaire qu'importe le nombre d'enfants`)
      messages.push(`Soumis à la CRDS (0.5%) : ${total.value}€ - ${CRDSCount(total.value)}€`)
    }
    return messages
  })
