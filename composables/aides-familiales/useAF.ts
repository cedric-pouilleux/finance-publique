...
export function useAF(options: FamilyPayload): FamilyReturn {
    ...
    const childsWith20 = computed((): Childrens[] =>
    ...
    const lumpSumAllowances = computed((): number => {
        ...
        if (!childsWith20.value.length || childrens.length < 3) {
            return 0;
        }
        const adjustedSum =
            resourcesTranch.value === 0
    ...
    const resume = computed((): string[] => {
        ...
        messages.push(
            `Vous etes dans la tranche ${currentTranch.value} ${currentTranchMessage.value}`
        );
        ...
        if (childrens.value!.length < 2) {
            messages.push(
                `Il faut au moin 2 enfants à charge pour bénéficier de l'allocation familiale`
            );
        } else {
            messages.push(
                `${childrens.value!.length} enfants à charge (${progressRate.value})`
            );
        }
        ...
        if (lumpSumAllowances.value) {
            ...
            if (!teenagesCanceled.value) {
                messages.push(
                    `${teenage.value.length} enfants à charge de plus de 14 ans (${
                        teenagesExpection.value ? 1 : teenage.value.length
                    } * ${RATE_CHILDREN.ageIncrease}%)`
                );
            }
        }
        ...
        if (sumWithTranch.value > 0) {
            ...
            messages.push(
                `Soumis à la CRDS (0.5%) : ${sumWithTranch.value}€ - ${CRDSCount(
                    sumWithTranch.value
                )}€ `
            );
        }
