...
export function useARS (options: FamilyPayload): FamilyReturn {
  ...
  const students = computed((): Childrens[] | undefined =>
  ...
  const allStudentsRateMessage = computed((): string[] | undefined =>
    students.value?.map(student => studentRate(student))
  )

  const rate = computed((): number =>
    students.value?.reduce((acc, child) => {
      const matched = SCHOOL_RATE.find(item => child.age >= item.age[0] && child.age <= item.age[1])
      return matched ? readdableRound(acc + matched.rate) : readdableRound(acc)
    }, 0) || 0
  )

  const maxSalary = computed((): number => SCHOOL_PRIME_TRANCHS + SCHOOL_TRANCHS_PER_CHILD * (childrens.value!.length - 1))
  ...
  const schoolAllowance = computed((): number => {
    if (!childrens.value && !salaries.value) {
      return 0
    }
